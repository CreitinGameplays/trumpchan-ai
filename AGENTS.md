# AGENTS.md

## Dev environment tips
- **Operating System:** This host is Windows. Always use **PowerShell commands** for terminal operations.
- **External APIs:** Always use web search when you are unsure about how to use any external API integrated into this project.

## Code style & conventions
- **Debugging:** Ensure to always add console debugging to your code. This makes it easier to troubleshoot and fix issues later if any arise.
- **Organization:** Please keep all source code files structured, clean, and organized.

## Documentation instructions
- **API Docs:** For every server API change or addition, you must update its documentation in the `API.md` file to make it easier to understand how the API works.